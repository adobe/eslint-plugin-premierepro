...
| [`no-async-in-empty-selection-type-checked`](docs/rules/no-async-in-empty-selection-type-checked.md)         | `no-async-in-empty-selection`     | Disallow async operations inside premierepro `TrackItemSelection.createEmptySelection()` callbacks                   |    error    |
| [`no-empty-selection-escape-type-checked`](docs/rules/no-empty-selection-escape-type-checked.md)             | `no-empty-selection-escape`       | Disallow the premierepro `createEmptySelection()` callback parameter from escaping the callback                      |    error    |
