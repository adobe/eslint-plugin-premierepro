...
export default createRule<[], MessageIds>({
  ...
  meta: {
    ...
    messages: {
      actionEscapesScope:
        "{{ methodName }}() result is assigned to a variable declared outside the " +
        "lock scope (lockedAccess() or executeTransaction() callback). Actions are only valid within the locked scope where " +
        "they were created.",
      actionEscapesViaProperty:
        "{{ methodName }}() result is stored on an object declared outside the " +
        "lock scope (lockedAccess() or executeTransaction() callback). Actions are only valid within the locked scope where " +
        "they were created.",
    },
    schema: [],
  ...
  create(context) {
    ...
    function isCreateActionCall(node: TSESTree.Node): node is TSESTree.CallExpression & {
      ...
      return (
        ...
        ACTION_PATTERN.test(node.callee.property.name)
      );
    }
    ...
    function checkAssignment(
      ...
      if (!isCreateActionCall(rhs)) return;

      const currentCallback = lockedCallbackStack[lockedCallbackStack.length - 1];
      const methodName = rhs.callee.property.name;
